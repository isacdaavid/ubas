...
class Subject(Member, Collection):
    ...
    def __repr__(self) -> str:
        ...
        some_member = list(self.members)[0]

        if isinstance(some_member, Datatype):
            return f"{classname}('{self.label}', datatypes={self.labels})"

        return f"{classname}('{self.label}', sessions={self.labels})"
